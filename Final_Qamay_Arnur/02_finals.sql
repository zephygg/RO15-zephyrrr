...
-- remove all privileges granted to the role first
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'vct_readonly') THEN
        REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA vct FROM vct_readonly;
        REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public FROM vct_readonly;
    END IF;

    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'vct_writer') THEN
        REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA vct FROM vct_writer;
        REVOKE ALL PRIVILEGES ON ALL TABLES IN SCHEMA public FROM vct_writer;
    END IF;
END $$;

-- drop roles first so script is re-runnable
DROP ROLE IF EXISTS vct_readonly;
DROP ROLE IF EXISTS vct_writer;

-- read-only role: can only view data
CREATE ROLE vct_readonly;

-- writer role: can insert and update match/roster data but not modify existing records arbitrarily
CREATE ROLE vct_writer;

-- allow read access to everything in vct schema
GRANT SELECT ON ALL TABLES IN SCHEMA vct TO vct_readonly;
GRANT SELECT ON ALL TABLES IN SCHEMA vct TO vct_writer;

-- allow writing to main tables (adjust names if needed)
GRANT INSERT, UPDATE ON teams TO vct_writer;
GRANT INSERT, UPDATE ON players TO vct_writer;
GRANT INSERT, UPDATE ON coaches TO vct_writer;
GRANT INSERT, UPDATE ON team_players TO vct_writer;
GRANT INSERT, UPDATE ON team_coaches TO vct_writer;

-- remove UPDATE privilege where strict data integrity is required (prevents altering historical roster records accidentally)
REVOKE UPDATE ON team_players FROM vct_writer;
REVOKE UPDATE ON team_coaches FROM vct_writer;